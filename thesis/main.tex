...
\renewenvironment{proof}[1][\proofname]{%
  \par\vspace{0pt}% Kein zusätzlicher Abstand davor
  \pushQED{\qed}%
  \normalfont%
  \topsep0pt \partopsep0pt % <--- hier wird Abstand nach dem Beweis unterdrückt
  \trivlist%
...
\titlespacing*{\section}
  {0pt}    % Einzug links
  {0pt}    % Abstand darüber
  {1ex}    % Abstand darunter

\allowdisplaybreaks % this will allow pagebreaking equation/align envs
  
\theoremstyle{break}
\newtheorem{thm}{Theorem}[section]

\theoremstyle{break}
\newtheorem{definthm}[thm]{Definition \& Theorem}
...
\nomenclature{$\mathcal{N}(G)$}{The set of open neighborhoods of the identity $e_G$ of a topological group $G$}
